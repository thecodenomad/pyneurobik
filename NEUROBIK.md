...
- TUI: Ratatui for interactive checkboxes/menus with async updates (use ratatui widgets for simple selection). Only shows models that haven't been downloaded yet (no confirmation file exists); OCI items always shown.
...
## Architecture
- `src/config.rs`: YAML parsing + serde validation + provider checks + env var expansion + provider confirmation file derivation.
- `src/downloader.rs`: Async downloads with reqwest/tokio, subprocess for Ollama/HF/podman, directory creation.
- `src/tui.rs`: Ratatui app with event loop for lists, checkboxes, progress (filters models by confirmation file existence).
- `src/main.rs`: CLI entry with ASCII art headers/footers (include themed boxes and neural art as shown in MVP), confirmation file creation (per-model and provider).
...
## Testing
- **Unit Tests**: Use cargo test for config deserialization (invalid YAML, schema mismatches), provider validation, provider confirmation file derivation, checksum verification, confirmation file creation (with mocked tokio::process for success/failure), model filtering by confirmation files.
- **Integration Tests**: Mock async downloads (reqwest), podman tokio::process, TUI events (ratatui test utils or manual), confirmation file filtering.
- **Edge Cases**: Resume interrupted downloads, podman build failures (invalid containerfile), large file handling, env var expansion, provider confirmation file creation.
- **NixOS Specific**: Cross-compile and test in NixOS; ensure Nix flake builds binary; verify service signaling via confirmation files (per-model and provider).
